...
import { X,History,Settings2,ArrowLeft,ArrowRight,Pencil,ArrowLeftRight,CircleHelp,Download,Plus,Check,WifiOff,Sun,RotateCw } from 'lucide-react';
import { createMatch,currentState,currentGame,gamesWon } from '../domain/engine';
import { teamName } from '../domain/participants';
import { ParticipantEditor } from '../ui/participant-editor';
...
type Panel='setup'|'settings'|'history'|'help'|'next'|'ends'|'names'|null;
export default function Home(){
 ...
 const titles={names:t("チーム名・選手名を編集"),setup:t("新しい試合"),settings:t("試合と表示の設定"),history:t("試合履歴"),help:t("RALLYの使い方"),next:t('GAME {game} の準備',{game:game.gameNumber+1}),ends:t("実際のエンドを交替")};
 return <div className="app-shell"><header className="app-header"><a className="brand" href="/" aria-label={t("RALLY ホーム")}><span className="brand-mark">R</span><span>RALLY<span className="brand-sub">UMPIRE SCOREBOARD</span></span></a><div className="header-actions"><span className={`local-status ${session.saveStatus==='error'?'failed':''}`} role="status"><i/>{session.saveStatus==='loading'?t("読込中"):session.saveStatus==='saving'?t("保存中"):session.saveStatus==='error'?t("未保存"):session.match?t("端末に保存済み"):t("新しい試合")}</span><button className="icon-button" aria-label={t("試合履歴")} title={t("試合履歴")} disabled={locked} onClick={()=>setPanel('history')}><History/></button><button className="icon-button" aria-label={t("設定")} title={t("設定")} disabled={locked} onClick={()=>setPanel('settings')}><Settings2/></button></div></header>
 ...
 <div className="match-bar"><div className="game-label">GAME <strong>{game.gameNumber.toString().padStart(2,'0')}</strong><span className="live-pill"><i/>{!session.match?t("開始前"):match.status==='completed'?t("試合終了"):state.winner?t("ゲーム終了"):pause?t("休憩中"):t("試合中")}</span></div><span className="rule-caption">{match.matchType==='doubles'?t("ダブルス"):t("シングルス")} <b>·</b> {t('{target}点 / {games}ゲーム先取',{target:match.rule.target,games:match.rule.gamesToWin})}</span></div>
 {state.winner&&<div className="result-strip" role="status"><div><strong>{match.winner?t('{team} の勝利',{team:teamName(match,match.winner)}):t('{team} がゲーム獲得',{team:teamName(match,state.winner)})}</strong><span>{match.winner?t('ゲーム {a}–{b}',{a:wins.A,b:wins.B}):`GAME ${game.gameNumber} · ${state.score.A}–${state.score.B}`}</span></div><button className="primary-button" disabled={locked} onClick={()=>setPanel(match.winner?'setup':'next')}>{match.winner?t("新しい試合"):t("次のゲームへ")}</button></div>}
 {pause&&<div className="notice"><div><strong>{t(pause.label)} <span className="timer">{Math.floor(remaining/60)}:{String(remaining%60).padStart(2,'0')}</span></strong><small>{remaining?t("準備ができたら再開できます。"):t("休憩時間が経過しました。")}</small></div><button disabled={locked} onClick={()=>run(session.resume)}>{t("休憩を終了")}</button></div>}
 {endNotice&&!pause&&<div className="notice"><span>{t("エンドを交替しました。得点カードも左右が変わります。")}</span><button onClick={()=>setEndNotice(false)}>{t("確認")}</button></div>}
 <ScoreCards match={match} state={state} flipped={session.prefs.flipped} disabled={!session.match||locked||!!pause||session.saveStatus==='error'} onScore={team=>run(()=>session.score(team))}/><p className="tap-hint">{state.winner?t("訂正するときは「戻る」"):pause?t("休憩を終了して得点入力を再開"):t("得点したチームのエリアをタップ")}</p>
 ...
 <Dialog open={panel!==null} onOpenChange={open=>{if(!open)setPanel(null)}}><DialogContent className="modal" showCloseButton={false}><DialogClose render={<button className="modal-close icon-button" aria-label={t("閉じる")}/> }><X/></DialogClose><DialogTitle>{panel?titles[panel]:''}</DialogTitle><DialogDescription>{panel==='names'?t("現在の試合の名前を変更します。"):panel==='setup'?t("開始後は得点した側をタップするだけ。"):panel==='history'?t("この端末に保存された試合を確認・再開できます。"):panel==='next'?t("サーバーとレシーバーを確認してください。"):t("審判の位置と、実際の試合に合わせて使用してください。")}</DialogDescription>
 ...
 {panel==='names'&&<ParticipantEditor match={match} onSave={async names=>{await session.rename(names);setPanel(null)}}/>}
 ...
 {panel==='settings'&&<div className="settings-list"><Choice label={t("言語")} value={language} options={[{value:"ja",label:"日本語"},{value:"en",label:"English"}]} onChange={value=>setLanguage(value as Language)}/>{languageError&&<p className="error-message" role="alert">{t("言語設定を保存できませんでした。")}</p>}<p className="form-note">{t('{name}：{target}点 / {margin}点差 / 上限{cap}点',{name:t(match.rule.name),target:match.rule.target,margin:match.rule.winBy,cap:match.rule.cap})}<br/>{match.rule.interval.at===null?t("ゲーム中の休憩なし"):t('{points}点で{seconds}秒の休憩',{points:match.rule.interval.at,seconds:match.rule.interval.seconds})} · {t('ゲーム間{seconds}秒',{seconds:match.rule.interval.betweenGamesSeconds})}</p><button className="secondary-button" disabled={!session.match} onClick={()=>setPanel('names')}><Pencil/>{t("チーム名・選手名を編集")}</button><button className="secondary-button" onClick={()=>setPanel('setup')}><Plus/>{t("新しい試合を設定")}</button><button className="secondary-button" onClick={()=>{run(session.flip);setPanel(null)}}><ArrowLeftRight/>{t("表示だけを180°反転")}</button><button className="secondary-button" disabled={!session.match||!!state.winner} onClick={()=>setPanel('ends')}><RotateCw/>{t("実際のエンドを交替")}</button><button className="secondary-button" disabled={!session.match} onClick={()=>downloadMatches([match])}><Download/>{t("現在の試合を書き出す")}</button><button className="secondary-button" onClick={()=>setPanel('help')}><CircleHelp/>{t("使い方・ホーム画面に追加")}</button><p className="form-note">{t("得点ルールは試合開始時に決定します。試合中の変更はできません。")}</p></div>}
 {panel==='ends'&&<div><p className="form-note">{t("選手が実際にコートの反対側へ移動した場合に使用します。サービス権とペアの右・左の担当は維持し、得点カードとコートを左右交替します。")}</p><div className="actions"><button className="secondary-button" onClick={()=>setPanel('settings')}>{t("戻る")}</button><button className="primary-button" onClick={()=>{run(session.ends);setPanel(null)}}>{t("エンド交替を記録")}</button></div></div>}
 {panel==='help'&&<div className="help-content"><p><strong>{t("1ラリー、1タップ。")}</strong><br/>{t("得点した側の大きなカードをタップします。サーブする選手を黄緑の「サーブ」、受ける選手を破線の「レシーブ」で表示します。")}</p><p><strong>{t("コートの見方")}</strong><br/>{t("ネットは中央の縦線です。右・左は各選手がネットを向いたときの向き。ダブルスの図はサービスコートの担当を示し、ラリー中の立ち位置を指定するものではありません。")}</p><p><strong>{t("訂正とエンド交替")}</strong><br/>{t("「戻る」で配置も含めて復元できます。次ゲームの開始直後に戻すと、前ゲームの最終得点を取り消します。エンド交替時は得点カードも移動するので、チーム名を確認してください。「進む」で取り消した得点をやり直します。表示反転は設定から操作できます。審判の表示は常にコート中央の下側です。")}</p><p><strong>{t("ホーム画面に追加")}</strong><br/>{t("iPhone：Safariの共有 →「ホーム画面に追加」。")}<br/>{t("Android：Chromeのメニュー →「アプリをインストール」または「ホーム画面に追加」。")}</p><p><strong>{t("オフラインと画面点灯")}</strong><br/>{pwa.offlineReady?t("オフライン利用の準備ができています。"):t("初回は通信できる状態で開き、アプリの保存が完了するまでお待ちください。")}{!import.meta.env.PROD?t(" 開発プレビューではオフライン保存を無効にしています。"):''}<br/>{pwa.wakeLocked?t("画面の点灯を維持しています。"):t("対応端末では試合中に画面の点灯を維持します。")}</p><p><strong>{t("試合データ")}</strong><br/>{t("この端末のブラウザ内に保存します。別の端末への移行やバックアップは「試合履歴」のJSON書き出しをご利用ください。ブラウザのサイトデータを削除すると記録も消えます。")}</p><p className="form-note">{t("15点制プリセット：15点・2点差・上限21点、8点で休憩。大会の要項に合わせて、開始前にルールを確認してください。")}</p></div>}
