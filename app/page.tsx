...
import { X,History,Settings2,ArrowLeft,ArrowRight,Pencil,CircleHelp,Download,Plus,Check,WifiOff,Sun,RotateCw } from 'lucide-react';
...
export default function Home(){
 ...
 const titles={names:t("チーム名・選手名を編集"),setup:t("新しい試合"),settings:t("試合と表示の設定"),history:t("試合履歴"),help:t("RALLYの使い方"),next:t('GAME {game} の準備',{game:game.gameNumber+1}),ends:t("コートチェンジ")};
 ...
 <ScoreCards match={match} state={state} disabled={!session.match||locked||session.saveStatus==='error'} onScore={team=>run(()=>session.score(team))}/><p className="tap-hint">{state.winner?t("訂正するときは「戻る」"):t("得点したチームのエリアをタップ")}</p>
 ...
 {panel==='settings'&&<div className="settings-list"><Choice label={t("言語")} value={language} options={[{value:"ja",label:"日本語"},{value:"en",label:"English"}]} onChange={value=>setLanguage(value as Language)}/>{languageError&&<p className="error-message" role="alert">{t("言語設定を保存できませんでした。")}</p>}<p className="form-note">{t('{name}：{target}点 / {margin}点差 / 上限{cap}点',{name:t(match.rule.name),target:match.rule.target,margin:match.rule.winBy,cap:match.rule.cap})}</p><button className="secondary-button" disabled={!session.match} onClick={()=>setPanel('names')}><Pencil/>{t("チーム名・選手名を編集")}</button><button className="secondary-button" onClick={()=>setPanel('setup')}><Plus/>{t("新しい試合を設定")}</button><button className="secondary-button" disabled={!session.match||!!state.winner} onClick={()=>setPanel('ends')}><RotateCw/>{t("コートチェンジ")}</button><button className="secondary-button" disabled={!session.match} onClick={()=>downloadMatches([match])}><Download/>{t("現在の試合を書き出す")}</button><button className="secondary-button" onClick={()=>setPanel('help')}><CircleHelp/>{t("使い方・ホーム画面に追加")}</button><p className="form-note">{t("得点ルールは試合開始時に決定します。試合中の変更はできません。")}</p></div>}
