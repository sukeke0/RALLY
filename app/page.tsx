import {OssLicenses} from '../ui/oss-licenses';
...
type Panel='licenses'|'edit'|'finish'|'setup'|'setup-warning'|'settings'|'history'|'help'|'next'|'ends'|'summary'|null;
export default function Home(){
 ...
 const titles={licenses:t('OSSライセンス'),summary:t('試合状況'),edit:t("試合設定を変更"),finish:t("ゲームセット"),setup:t("新しい試合"),'setup-warning':t("新しい試合"),settings:t("試合と表示の設定"),history:t("試合履歴"),help:t("RALLYの使い方"),next:t('GAME {game} の準備',{game:game.gameNumber+1}),ends:t("コートチェンジ")};
 ...
 {!session.match&&session.saveStatus!=='loading'&&<div className="start-strip"><span>{t("選手とルールを設定して始めましょう")}</span><button className="primary-button" disabled={locked} onClick={openSetup}><Plus/>{t("試合を設定")}</button></div>}
 ...
 <div className="match-bar"><div className="game-label"><span>GAME {game.gameNumber}</span><span className="live-pill"><i/>{!session.match?t("開始前"):match.status==='completed'?t("試合終了"):state.finished?t("セット終了"):t("試合中")}</span></div><span className="rule-caption">{match.matchType==='doubles'?t("ダブルス"):t("シングルス")} <b>·</b> {t('{target}点 / {games}ゲーム先取',{target:match.rule.target,games:match.rule.gamesToWin})}</span></div>
 ...
 <footer className="board-footer"><button onClick={()=>setPanel('help')}><CircleHelp size={15}/>{t("RALLYの使い方")}</button><span className="device-status">{!pwa.online?<WifiOff size={13}/>:pwa.offlineReady?<Check size={13}/>:null}{pwa.wakeLocked&&<Sun size={13}/>}</span></footer>
 </main>
 ...
 <Dialog open={panel!==null&&!pendingEnds} onOpenChange={open=>{if(!open)setPanel(null)}}><DialogContent className="modal" showCloseButton={false}><DialogClose render={<button className="modal-close icon-button" aria-label={t("閉じる")}/> }><X/></DialogClose><DialogTitle>{panel?titles[panel]:''}</DialogTitle>{!['settings','help','ends','summary','setup','licenses'].includes(panel??'')&&<DialogDescription>{panel==='setup-warning'?t('新しい試合を開始すると、現在の試合と得点履歴は削除されます。元に戻せません。'):panel==='edit'?t('現在の試合の設定を修正します。'):panel==='finish'?t('棄権や時間切れなどによる途中終了を記録します。'):panel==='history'?t("この端末に保存された試合を確認・再開できます。"):panel==='next'?t("サーバーとレシーバーを確認してください。"):null}</DialogDescription>}
 ...
 {panel==='settings'&&<div className="settings-list"><Choice label={t("言語/Language")} value={language} options={[{value:"ja",label:"日本語"},{value:"en",label:"English"}]} onChange={value=>setLanguage(value as Language)}/>{languageError&&<p className="error-message" role="alert">{t("言語設定を保存できませんでした。")}</p>}<p className="form-note">{t('{name}：{target}点 / {margin}点差 / 上限{cap}点',{name:t(match.rule.name),target:match.rule.target,margin:match.rule.winBy,cap:match.rule.cap})}</p><button className="secondary-button" disabled={!session.match} onClick={()=>setPanel('edit')}><Settings2/>{t("試合設定を変更")}</button><button className="secondary-button" onClick={openSetup}><Plus/>{t("新しい試合を設定")}</button><button className="secondary-button" disabled={!session.match||state.finished} onClick={()=>setPanel('ends')}><RotateCw/>{t("コートチェンジ")}</button><button className="secondary-button" disabled={!session.match||state.finished} onClick={()=>setPanel('finish')}><Flag/>{t("ゲームセット")}</button><button className="secondary-button" disabled={!session.match} onClick={()=>downloadMatches([match])}><Download/>{t("現在の試合を書き出す")}</button><button className="secondary-button" onClick={()=>setPanel('help')}><CircleHelp/>{t("使い方・ホーム画面に追加")}</button><button className="oss-license-link" onClick={()=>setPanel('licenses')}>{t("OSSライセンス")}</button></div>}
