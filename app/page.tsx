...
import { X,Flag,History,Settings2,ArrowLeft,ArrowRight,Pencil,CircleHelp,Download,Plus,Check,WifiOff,Sun,RotateCw } from 'lucide-react';
...
type Panel='edit'|'finish'|'setup'|'settings'|'history'|'help'|'next'|'ends'|'names'|null;
export default function Home(){
 ...
 const titles={edit:t("試合設定を変更"),finish:t("ゲームセット"),names:t("チーム名・選手名を編集"),setup:t("新しい試合"),settings:t("試合と表示の設定"),history:t("試合履歴"),help:t("RALLYの使い方"),next:t('GAME {game} の準備',{game:game.gameNumber+1}),ends:t("コートチェンジ")};
 ...
 <Dialog open={panel!==null} onOpenChange={open=>{if(!open)setPanel(null)}}><DialogContent className="modal" showCloseButton={false}><DialogClose render={<button className="modal-close icon-button" aria-label={t("閉じる")}/> }><X/></DialogClose><DialogTitle>{panel?titles[panel]:''}</DialogTitle><DialogDescription>{panel==='edit'?t('現在の試合の設定を修正します。'):panel==='finish'?t('棄権や時間切れなどによる途中終了を記録します。'):panel==='names'?t("現在の試合の名前を変更します。"):panel==='setup'?t("開始後は得点した側をタップするだけ。"):panel==='history'?t("この端末に保存された試合を確認・再開できます。"):panel==='next'?t("サーバーとレシーバーを確認してください。"):t("審判の位置と、実際の試合に合わせて使用してください。")}</DialogDescription>
 ...
 {panel==='settings'&&<div className="settings-list"><Choice label={t("言語")} value={language} options={[{value:"ja",label:"日本語"},{value:"en",label:"English"}]} onChange={value=>setLanguage(value as Language)}/>{languageError&&<p className="error-message" role="alert">{t("言語設定を保存できませんでした。")}</p>}<p className="form-note">{t('{name}：{target}点 / {margin}点差 / 上限{cap}点',{name:t(match.rule.name),target:match.rule.target,margin:match.rule.winBy,cap:match.rule.cap})}</p><button className="secondary-button" disabled={!session.match} onClick={()=>setPanel('names')}><Pencil/>{t("チーム名・選手名を編集")}</button><button className="secondary-button" disabled={!session.match} onClick={()=>setPanel('edit')}><Settings2/>{t("試合設定を変更")}</button><button className="secondary-button" disabled={!session.match||state.finished} onClick={()=>setPanel('finish')}><Flag/>{t("ゲームセット")}</button><button className="secondary-button" onClick={()=>setPanel('setup')}><Plus/>{t("新しい試合を設定")}</button><button className="secondary-button" disabled={!session.match||state.finished} onClick={()=>setPanel('ends')}><RotateCw/>{t("コートチェンジ")}</button><button className="secondary-button" disabled={!session.match} onClick={()=>downloadMatches([match])}><Download/>{t("現在の試合を書き出す")}</button><button className="secondary-button" onClick={()=>setPanel('help')}><CircleHelp/>{t("使い方・ホーム画面に追加")}</button></div>}
