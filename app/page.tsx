...
export default function Home(){
 ...
 <div className="match-bar"><div className="game-label">GAME <strong>{game.gameNumber.toString().padStart(2,'0')}</strong><span className="live-pill"><i/>{!session.match?t("開始前"):match.status==='completed'?t("試合終了"):state.winner?t("ゲーム終了"):t("試合中")}</span></div><span className="rule-caption">{match.matchType==='doubles'?t("ダブルス"):t("シングルス")} <b>·</b> {t('{target}点 / {games}ゲーム先取',{target:match.rule.target,games:match.rule.gamesToWin})}</span></div>
 ...
 <ScoreCards match={match} state={state} flipped={session.prefs.flipped} disabled={!session.match||locked||session.saveStatus==='error'} onScore={team=>run(()=>session.score(team))}/><p className="tap-hint">{state.winner?t("訂正するときは「戻る」"):t("得点したチームのエリアをタップ")}</p>
