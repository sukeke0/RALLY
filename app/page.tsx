...
import { History,Settings2,ArrowLeft,ArrowRight,Pencil,ArrowLeftRight,CircleHelp,Download,Plus,Check,WifiOff,Sun,RotateCw } from 'lucide-react';
...
export default function Home(){
 ...
 const titles={names:'チーム名・選手名を編集',setup:'新しい試合',settings:'試合と表示の設定',history:'試合履歴',help:'RALLYの使い方',next:`GAME ${game.gameNumber+1} の準備`,ends:'実際のエンドを交替'};
 ...
 <ScoreCards match={match} state={state} flipped={session.prefs.flipped} disabled={!session.match||locked||!!pause||session.saveStatus==='error'} onScore={team=>run(()=>session.score(team))}/><p className="tap-hint">{state.winner?'訂正するときは「戻る」':pause?'休憩を終了して得点入力を再開':'得点したチームのエリアをタップ'}</p>
 <ScoreSheet match={match}/><Court match={match} state={state} flipped={session.prefs.flipped}/>
 <div className="toolbar"><button className="undo-button" title="直前の得点を取り消す" disabled={!session.canUndo||locked} onClick={()=>{setEndNotice(false);run(session.undo)}}><ArrowLeft/>戻る</button><button className="undo-button" title="取り消した得点をやり直す" disabled={!session.canRedo||locked} onClick={()=>run(session.redo)}>進む<ArrowRight/></button></div>
 ...
 <Dialog open={panel!==null} onOpenChange={open=>{if(!open)setPanel(null)}}><DialogContent className="modal"><DialogTitle>{panel?titles[panel]:''}</DialogTitle><DialogDescription>{panel==='names'?'現在の試合の名前を変更します。':panel==='setup'?'開始後は得点した側をタップするだけ。':panel==='history'?'この端末に保存された試合を確認・再開できます。':panel==='next'?'サーバーとレシーバーを確認してください。':'審判の位置と、実際の試合に合わせて使用してください。'}</DialogDescription>
