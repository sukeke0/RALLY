...
export function registerScoreTools(actions:{read:()=>unknown;score:(team:Team)=>Promise<unknown>;undo:()=>Promise<unknown>},context?:Context){
 ...
 const tools:Tool[]=[
  ...
  {name:'record_rally',title:'1ラリーの得点を記録',description:'Add one point to the rally winner in the active match, update the visible scoreboard, and save on this device. Fails during a pause or after game end.',inputSchema:{type:'object',properties:{winner:{type:'string',enum:['A','B']}},required:['winner'],additionalProperties:false},annotations:{readOnlyHint:false,untrustedContentHint:true},execute:input=>{const x=input as {winner:Team};if(!x||!['A','B'].includes(x.winner)||Object.keys(x).some(k=>k!=='winner'))throw new Error('winner must be A or B');return actions.score(x.winner)}},
