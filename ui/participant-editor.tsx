...
export function ParticipantFields({names,matchType,onChange}:{names:ParticipantNames;matchType:MatchType;onChange:(names:ParticipantNames)=>void}){
 ...
 return <div className="form-grid">{(['A','B'] as const).map(team=><div key={team} className="player-inputs">
  ...
  {([`${team}1`,...(matchType==='doubles'?[`${team}2`]:[])] as (keyof ParticipantNames['players'])[]).map(id=><label className="field" key={id}>{t('{id} 選手名',{id})}<input maxLength={40} placeholder={id} value={names.players[id]} onChange={e=>onChange({...names,players:{...names.players,[id]:e.target.value}})}/></label>)}
 </div>)}</div>;
}
