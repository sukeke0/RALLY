...
import { courtPositions,currentGame } from '../domain/engine';
...
export function Court({match,state}:{match:Match;state:GameState}){
 ...
 const positions=courtPositions(match,state),firstServer=match.matchType==='doubles'?currentGame(match).initialServer:null,firstReceiver=match.matchType==='doubles'?currentGame(match).initialReceiver:null;
 useLayoutEffect(()=>{
