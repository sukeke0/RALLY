...
export function ScoreSheet({match}:{match:Match}) {
 ...
 return <section className="sheet-section"><div className="section-heading"><h2>{t("得点経過")}</h2><div className="sheet-meta">{match.games.length>1&&<div className="game-switch">{match.games.map((g,i)=><button key={i} aria-pressed={selected===i} onClick={()=>setSelected(i)}>G{g.gameNumber}</button>)}</div>}<span className="desktop-only">{t("横にスクロール →")}</span></div></div><div className="score-sheet" ref={ref}><Table aria-label={t('ゲーム{game}の得点経過',{game:game.gameNumber})}><TableBody>{(['A','B'] as const).map(team=><TableRow key={team}><TableCell className={`row-label team-${team.toLowerCase()}`} role="rowheader">{team}</TableCell>{Array.from({length:cells},(_,i)=>{const rally=game.rallyHistory[i];return <TableCell key={i} className={`${rally?.winner===team?'scored':''} ${i===count-1?'latest':''}`} title={rally?t('ラリー{rally}：{a}–{b}',{rally:i+1,a:rally.scoreAfter.A,b:rally.scoreAfter.B}):undefined}>{rally?.winner===team?rally.scoreAfter[team]:''}</TableCell>;})}</TableRow>)}</TableBody></Table></div>{game.ending&&<p className="form-note">{t(endingReason(game.ending.reason))}</p>}</section>;
}
