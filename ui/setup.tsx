...
export function Setup({previous,onStart}:{previous?:Match;onStart:(setup:MatchSetup)=>Promise<void>}){
 ...
 <div className="form-section"><div className="form-grid"><Choice label={t("種目")} value={type} options={[{value:'doubles',label:t("ダブルス")},{value:'singles',label:t("シングルス")}]} onChange={v=>{setType(v as MatchType);setAFirst('A1');setBFirst('B1');}}/><Choice label={t("得点ルール")} value={rule.id} options={[{value:'21',label:t("21点制")},{value:'15',label:t("15点制")},{value:'custom',label:t("カスタム")}]} onChange={v=>setRule(structuredClone(v==='21'?RULE_21:v==='15'?RULE_15:{...rule,id:'custom',name:'カスタム'}))}/></div><p className="form-note">{t('{target}点・{margin}点差・上限{cap}点 / {games}ゲーム先取',{target:rule.target,margin:rule.winBy,cap:rule.cap,games:rule.gamesToWin})}</p></div>
 {rule.id==='custom'&&<div className="form-section"><div className="form-grid">{numberField(t("基本得点"),rule.target,v=>setRule({...rule,target:v}),99,1)}{numberField(t("デュース時の点差"),rule.winBy,v=>setRule({...rule,winBy:v}),99,1)}{numberField(t("最大得点"),rule.cap,v=>setRule({...rule,cap:v}),199,1)}{numberField(t("勝利に必要なゲーム数"),rule.gamesToWin,v=>setRule({...rule,gamesToWin:v}),5,1)}{numberField(t("休憩に入る得点（0＝なし）"),rule.interval.at??0,v=>setRule({...rule,interval:{...rule.interval,at:v||null}}),98)}{numberField(t("ゲーム中の休憩（秒）"),rule.interval.seconds,v=>setRule({...rule,interval:{...rule.interval,seconds:v}}),600)}{numberField(t("ゲーム間の休憩（秒）"),rule.interval.betweenGamesSeconds,v=>setRule({...rule,interval:{...rule.interval,betweenGamesSeconds:v}}),600)}{numberField(t("最終ゲームの交替点（0＝なし）"),rule.ends.decidingGameAt??0,v=>setRule({...rule,ends:{...rule.ends,decidingGameAt:v||null}}),98)}</div><label className="check-field"><Checkbox checked={rule.ends.betweenGames} onCheckedChange={v=>setRule({...rule,ends:{...rule.ends,betweenGames:!!v}})}/>{t("ゲーム間にエンド交替する")}</label></div>}
...
export function NextGameSetup({match,onStart}:{match:Match;onStart:(server:PlayerId,receiver:PlayerId)=>Promise<void>}){
 ...
 return <form onSubmit={async e=>{e.preventDefault();setBusy(true);try{await onStart(server,receiver)}catch(err){setError((err as Error).message)}finally{setBusy(false)}}}><div className="form-section"><p className="form-note">{t('前ゲームの勝者 {team} がサーブします。',{team:teamName(match,winner)})}{match.rule.ends.betweenGames?t("エンドは自動で交替します。"):''}</p><Choice label={t("最初のサーバー")} value={server} options={options(winner)} onChange={v=>setServer(v as PlayerId)}/><Choice label={t("最初のレシーバー")} value={receiver} options={options(other)} onChange={v=>setReceiver(v as PlayerId)}/></div>{error&&<p className="error-message">{t(error)}</p>}<button className="primary-button" disabled={busy}>{t('GAME {game} を開始',{game:match.games.length+1})}</button></form>;
}
