...
export function Setup({previous,editing=false,onStart}:{previous?:Match;editing?:boolean;onStart:(setup:MatchSetup)=>Promise<void>}){
 ...
 const clear=()=>{setType('doubles');setPlayers({A1:'',A2:'',B1:'',B2:''});setTeamNames({A:'',B:''});setRule(structuredClone(RULE_15));setServing('A');setAFirst('A1');setBFirst('B1');setSide('left');setError('');};
 ...
 <div className="setup-form-actions"><button className="secondary-button" type="button" disabled={busy} onClick={clear}><RotateCcw/>{t('設定をクリア')}</button></div>
 ...
 <div className="form-section"><h3>{t("チーム名・選手名")}</h3><ParticipantFields names={{teamNames,players}} matchType={type} onChange={names=>{setTeamNames(names.teamNames);setPlayers(names.players)}}/></div>
 <div className="form-section"><h3>{editing?t("第1ゲーム開始時のサービスと配置"):t("初期配置")}</h3><div className="form-grid"><Choice label={t("最初にサーブするチーム")} value={serving} options={[{value:'A',label:teamNames.A||'Team A'},{value:'B',label:teamNames.B||'Team B'}]} onChange={v=>setServing(v as Team)}/><Choice label={t("審判から見たTeam Aの側")} value={side} options={[{value:'left',label:t("左側")},{value:'right',label:t("右側")}]} onChange={v=>setSide(v as Side)}/><Choice label={t("最初のサーバー")} value={serving==='A'?aFirst:bFirst} options={personOptions(serving)} onChange={v=>serving==='A'?setAFirst(v as PlayerId):setBFirst(v as PlayerId)}/><Choice label={t("最初のレシーバー")} value={serving==='A'?bFirst:aFirst} options={personOptions(opponent(serving))} onChange={v=>serving==='A'?setBFirst(v as PlayerId):setAFirst(v as PlayerId)}/></div></div>
