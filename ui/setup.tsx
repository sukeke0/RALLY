...
export function Setup({previous,editing=false,onStart}:{previous?:Match;editing?:boolean;onStart:(setup:MatchSetup)=>Promise<void>}){
 ...
 const [side,setSide]=useState<Side>(first?.initialTeamASide??'left'),[rule,setRule]=useState<Rule>(structuredClone(editing&&previous?{...previous.rule,interval:{at:null,seconds:0,betweenGamesSeconds:0}}:RULE_21));
 ...
 {error&&<p className="error-message" role="alert">{t(error)}</p>}<button className="primary-button" disabled={busy} type="submit">{busy?t("保存中…"):editing?t("変更を保存"):t("この設定で試合を開始")}</button><p className="form-note" style={{marginTop:10}}>{editing?t("得点履歴を保って配置を再計算します。記録と矛盾する変更は保存できません。"):t("進行中の試合も履歴に残ります。")}</p></form>;
}
