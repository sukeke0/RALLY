...
export function Setup({previous,editing=false,onStart}:{previous?:Match;editing?:boolean;onStart:(setup:MatchSetup)=>Promise<void>}){
 ...
 if(pendingSetup&&previous)return <div className="replace-match-confirm"><p className="error-message" role="alert">{t('新しい試合を開始すると、現在の試合と得点履歴は削除されます。元に戻せません。')}</p><p className="form-note">{teamName(previous,'A')} {t('対')} {teamName(previous,'B')} · {currentState(previous).score.A}–{currentState(previous).score.B}</p>{error&&<p className="error-message" role="alert">{t(error)}</p>}<div className="actions"><button className="secondary-button" disabled={busy} onClick={()=>{setPendingSetup(null);setError('')}}>{t('キャンセル')}</button><button className="primary-button delete-confirm-button" disabled={busy} onClick={()=>void start(pendingSetup)}>{busy?t('保存中…'):t('破棄して新しい試合を開始')}</button></div></div>;
 return <form onSubmit={e=>{e.preventDefault();setError('');try{validateRule(rule);const setup:MatchSetup={matchType:type,players,teamNames,rule,servingTeam:serving,server:serving==='A'?aFirst:bFirst,receiver:serving==='A'?bFirst:aFirst,teamASide:side};if(!editing&&previous)setPendingSetup(setup);else void start(setup);}catch(err){setError((err as Error).message);}}}>
