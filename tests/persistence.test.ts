...
test('IndexedDB commit and a fresh repository restore complete match and preferences',async()=>{
 ...
 const repo=new MatchRepository(dbName);const prefs={matchId:m.matchId,flipped:true,pause:null};await repo.save(m,prefs);await repo.close();
...
test('invalid bulk import is atomic and duplicate ID never overwrites a match',async()=>{
 const repo=new MatchRepository(crypto.randomUUID()),m=make();await repo.save(m,{matchId:m.matchId,flipped:false,pause:null});
