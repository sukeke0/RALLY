...
function fixture(team){
 let m=createMatch({matchType:'doubles',players:{A1:'John',A2:'James',B1:'Bob',B2:'Ben'},rule:RULE_21,servingTeam:'A',server:'A1',receiver:'B1',teamASide:'left'},crypto.randomUUID(),now);
 for(let i=0;i<21;i++)m=scorePoint(m,'A',now);
 m=nextGame(m,'A1','B1',now);
 for(let i=0;i<21;i++)m=scorePoint(m,'B',now);
 m=nextGame(m,'B2','A2',now);
 for(let i=0;i<10;i++)m=scorePoint(m,team,now);
 return m;
}
try{
 for(const [language,team,answer] of [['ja','A','Yes'],['en','B','No']]){
  ...
  await page.screenshot({path:`outputs/end-decision-${language}.png`});
  ...
  await page.getByRole('button',{name:language==='ja'?'戻る':'Back',exact:true}).click();await saved();assert.equal(await left(),before);assert.equal(await card().locator('.score-number').textContent(),'10');
  ...
  await context.close();console.log(`${language}: ${answer}, pending reload, saved answer, Undo/Redo and alternate answer passed`);
 }
