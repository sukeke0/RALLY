...
try{
 ...
 await page.getByRole('button',{name:'この設定で試合を開始',exact:true}).click();
 ...
 assert.equal(await page.getByRole('combobox',{name:'種目',exact:true}).textContent(),'シングルス▼');
 ...
 await page.getByRole('button',{name:'設定',exact:true}).click();assert.equal(await page.getByRole('button',{name:'現在の試合を破棄',exact:true}).count(),0);await page.getByRole('button',{name:'新しい試合を設定',exact:true}).click();await choose('種目','シングルス');
 await page.getByRole('button',{name:'この設定で試合を開始',exact:true}).click();await replaceWarning.waitFor();
 ...
 await page.getByRole('button',{name:'破棄して新しい試合を開始',exact:true}).click();await page.getByText('新しい試合を保存できませんでした。現在の試合は保持されています。',{exact:true}).waitFor();assert.equal(await page.locator('.score-card.team-a .score-number').textContent(),'20');
 await page.screenshot({path:'outputs/replace-match-warning.png'});
 await page.getByRole('button',{name:'破棄して新しい試合を開始',exact:true}).click();await saved();await page.getByRole('dialog').waitFor({state:'hidden'});await page.reload();await saved();assert.equal(await page.locator('.score-card.team-a .score-number').textContent(),'0');
 ...
 await page.getByRole('button',{name:'閉じる',exact:true}).click();await page.getByRole('button',{name:'設定',exact:true}).click();await choose('言語','English');await page.getByRole('button',{name:'Set up a new match',exact:true}).click();await page.getByRole('button',{name:'Start match',exact:true}).click();
 await page.getByText('Starting a new match deletes the current match and its rally history. This cannot be undone.',{exact:true}).waitFor();await page.getByRole('button',{name:'Discard and start new match',exact:true}).click();await page.getByText('Saved on device',{exact:true}).waitFor();await page.getByRole('dialog').waitFor({state:'hidden'});
